...
int halide_hexagon_remote_set_performance_v2(
    int set_mips,
    ...
    request.dcvs_v2.dcvs_params.target_corner = voltageCorner[mode];
    request.dcvs_v2.set_latency     = set_latency;
    request.dcvs_v2.latency         = latency;
    retval = HAP_power_set(NULL, &request);
    if (0 != retval) {
        log_printf("HAP_power_set(HAP_power_set_dcvs_v2) failed (%d)\n", retval);
        memset(&request, 0, sizeof(HAP_power_request_t));
...
int halide_hexagon_remote_set_performance_mode(int mode) {
    ...
    return halide_hexagon_remote_set_performance_v2(set_mips,
                                                 mipsPerThread,
                                                 mipsTotal,
                                                 set_bus_bw,
                                                 bwMegabytesPerSec,
